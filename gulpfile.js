...
gulp.task("compile-inline-less", function() {
    let template_compiler = "export const inlineCss = \"{{{template}}}\";";
    return gulp.src(["./src/entitylayoutinline.less"])
        .pipe(less({ style: 'compressed' }).on('error', util.log))
        //.pipe(autoprefix('last 10 version'))
        .pipe(cssmin().on('error', util.log))
        .pipe(intercept(function(file) {
            let content = mustache.render(template_compiler, {
                template: file.contents.toString()
            });
            file.contents = new Buffer(content);
            return file;
        }))
        .pipe(rename('entitylayoutinlinecss.ts'))
        .pipe(gulp.dest('./src'));
});
...
// Default Task
gulp.task('default', ["compile-inline-less", "lint", "bundle", "less"]);